...
function formatDemoDateTime(scheduledAt, timeZone = TZ) {
...
function getTimezoneLabel(timeZone = TZ) {
  try {
...
async function sendConfirmationEmail({ name, email, scheduledAt, clientTimezone }) {
  ...
  const tz = clientTimezone && isValidTimezone(clientTimezone) ? clientTimezone : TZ
  const { dateLabel, timeLabel } = formatDemoDateTime(scheduledAt, tz)
    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:auto;padding:24px">
          ...
          <p style="font-size:18px"><strong>${escapeHtml(dateLabel)}</strong><br>${escapeHtml(timeLabel)} (${escapeHtml(tzLabel)})</p>
