...
for (let minutes = 9 * 60; minutes <= 20 * 60; minutes += 30) {
  ...
  const m = minutes % 60
  ALL_SLOT_TIMES.push(`${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`)
}
...
function buildSlotsForDate(dateStr, bookedKeys) {
  ...
  return ALL_SLOT_TIMES.map((time) => {
    ...
    const available = !tooSoon && !booked && !fakeBusy
    return {
      time,
      label: formatTimeLabel(time),
      available,
      reason: tooSoon ? 'too_soon' : booked ? 'booked' : fakeBusy ? 'busy' : 'open',
    }
  })
}
...
function buildAvailability(bookedKeys = new Set()) {
  ...
  const days = dates.map((date) => {
    const slots = buildSlotsForDate(date, bookedKeys)
    return {
      date,
      slots,
...
function isSlotAvailable(dateStr, timeStr, bookedKeys) {
  const slots = buildSlotsForDate(dateStr, bookedKeys)
