/* Datos embebidos (sin ES module) para compatibilidad en hosting estático */
const kalyCategories = [
...
function renderDemos() {
  ...
  kalyCategories.forEach((cat, index) => {
    const title = document.createElement('h3');
    title.className = 'kaly-category-title';
    title.id = `kaly-cat-${cat.id}`;
    title.textContent = cat.label;
    if (index === 0) title.classList.add('kaly-category-title--first');
